...
"""
import sys
sys.path.append('/Dataset/mas-chen.weidong/project_c/valle/vall-e/')
sys.path.append('/Dataset/mas-chen.weidong/project_c/valle/vall-e/egs/libritts/bin/')
import argparse
import logging
import os
...
# token_dict = kaldiio.load_scp('libri_token_faiss.scp')
...
def get_args():
    ...
    parser.add_argument("--tokenscp",
                        type = str,
                        default = 'libritts_wavlm_faiss.scp')
...
def main():
    ...
    token_dict = kaldiio.load_scp(args.tokenscp)
    mel_dict = kaldiio.load_scp(args.mels)
    ...
    if dataset_parts == "all":  # LibriTTS
        ...
        print('dataset_parts: ', dataset_parts)
    else:
    ...
    #             audio_id1 = c.id
    #             audio_id2 = c.recording.id
    #             print('audio', audio_id1, audio_id2)
    #         assert 0 == 1
    with get_executor() as ex:
        for partition, m in manifests.items():
            logging.info(
            ...
            if args.text_extractor:
                if (
                    ...
                    for c in tqdm(cut_set):
                        id = c.recording.id
                        predict_token = token_dict[id]
                        list_predict_token = predict_token.tolist()
                        mels = mel_dict[id].tolist()
                        ...
                        c.supervisions[0].custom['wav2vec'] = {"wav2vec": list_predict_token}
                        c.supervisions[0].custom['mel'] = {"mel": mels}
                        unique_symbols.update(phonemes)
