...
from optim2 import Eden, ScaledAdam
...
def generate_binary_matrix(input_vector):
    # 获取输入向量的长度n和最大值m
    n = len(input_vector)
    m = max(input_vector)
    
    # 创建一个全零矩阵
    binary_matrix = torch.zeros(n, m)
    another_matrix = torch.zeros(n, 1)
    # 生成一个索引矩阵，其第i行前input_vector[i]个元素为1
    idx_matrix = torch.arange(m).expand(n, -1) < input_vector.unsqueeze(1)
    
    # 使用索引矩阵填充binary_matrix中的1
    binary_matrix[idx_matrix] = 1
    binary_matrix = torch.cat([another_matrix, binary_matrix], -1)
...
def compute_loss(
    ...
    token_seq = token_seq.squeeze(1)
    
    text_lens = batch["text_len"].to(device)
    token_lens = batch["token_len"]
    ref_mels = batch["ref_mels"].to(device)

    pad_seq = generate_binary_matrix(token_lens)
    token_seq = token_seq + pad_seq
    ...
    with torch.set_grad_enabled(is_training):
        simple_loss, pruned_loss = model(
            ...
            reference_audio = ref_mels,
            true_seq = token_seq[:, 1:],
            prune_range=50,
    ...
    with warnings.catch_warnings():
        ...
        info['frames'] = (token_lens).sum().item()
        info['utterances'] = text_seq.size(0)
...
def run(rank, world_size, args):
    ...
    librispeech = TtsDataModule(args)
    ...
    train_cuts = librispeech.dev_cuts()
    # train_cuts = train_cuts.filter(remove_short_and_long_utt)
    train_cuts = filter_short_and_long_utterances(train_cuts, params.filter_min_duration, params.filter_max_duration)
    ...
    valid_cuts = filter_short_and_long_utterances(valid_cuts, params.filter_min_duration, params.filter_max_duration)
    ...
    for epoch in range(params.start_epoch, params.num_epochs + 1):
        ...
        logging.info('begin epoch training!')
        train_one_epoch(
            params=params,
            model=model,
            model_avg=model_avg,
